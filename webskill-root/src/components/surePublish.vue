...
<template>
    <div class="surePublish row">
        <p class="navbar-header col-xs-12">
          <span class="navbar-brand col-xs-6 pull-left" :class="{cur:myPostList==0}"  @click="postPublishType(0)">待发布{{error}}</span>
          <span class="navbar-brand col-xs-6 pull-left" :class="{cur:myPostList==1}" @click="postPublishType(1)">已发布{{errorHas}}</span>
        </p>
